"""
app.py — SigmaFetcher V10  (network-error fixed)
All routes that are called via fetch() always return JSON — never HTML.
"""
...
from flask import (Flask, render_template, request, redirect,
                   url_for, session, jsonify, flash, abort, make_response)
...
def create_app():
    ...
    # ── Config ────────────────────────────────────────────────────────
    app.secret_key = os.getenv("SECRET_KEY", "sigma-v10-xK9mP2qRjL7wNdQ4")
    ...
    # ── KEY FIX: Any unhandled exception on an API route returns JSON ──
    @app.errorhandler(Exception)
    def handle_exception(e):
        # API / fetch() calls: return JSON error so JS doesn't get "Network error"
        if (request.path.startswith("/api/") or
                request.path in ("/login", "/signup") or
                request.headers.get("Content-Type", "").startswith("application/json") or
                request.headers.get("X-Requested-With") == "XMLHttpRequest"):
            log.error("API error on %s: %s", request.path, traceback.format_exc())
            return jsonify({"ok": False, "error": f"Server error: {str(e)}"}), 500
        # Regular page: show simple HTML error
        return (f"<h2 style='font-family:monospace;color:#ef4444;padding:2rem'>"
                f"Error: {e}<br><a href='/'>← Home</a></h2>"), 500
    ...
    def not_found(e):
        ...
        return (f"<h2 style='font-family:monospace;color:#ef4444;padding:2rem'>"
                f"404 — Not found<br><a href='/'>← Home</a></h2>"), 404
    ...
    def forbidden(e):
        ...
        return (f"<h2 style='font-family:monospace;color:#ef4444;padding:2rem'>"
                f"403 — Access denied<br><a href='/'>← Home</a></h2>"), 403
    ...
    def admin_required(f):
        ...
        def w(*a, **kw):
            ...
            if not u.is_admin:
                return (f"<h2 style='font-family:monospace;color:#ef4444;padding:2rem'>"
                        f"Admin only — <a href='/'>← Home</a></h2>"), 403
            return f(*a, **kw)
        return w
    ...
    def login_route():
        try:
            ...
            return jsonify({"ok": False, "error": "Server error during login"})
    ...
    def signup_route():
        try:
            ...
            return jsonify({"ok": False, "error": "Server error during signup"})
    ...
    def api_get_number(nid):
        ...
        order = Order(
            user_id=user.id, virtual_number_id=vn.id,
    ...
    def api_sms(nid):
        ...
        # Live fetch — never crash even if panel is offline
        if vn.panel and vn.panel.active:
            try:
                client = get_client(vn.panel)
                fresh  = client.fetch_sms(vn.number)
                for m in fresh:
                ...
                log.warning("fetch_sms failed for number %s: %s", vn.number, e)
    ...
    def admin_route():
        ...
        # Safely enrich to avoid template crashes
        for n in numbers:
    ...
    def admin_add_panel():
        ...
        p = Panel(name=name, base_url=url, username=uname,
    ...
    def admin_panel_toggle(pid):
        ...
        db.session.commit(); evict_client(pid)
    ...
    def admin_panel_ping(pid):
        p = db.session.get(Panel, pid)
        if not p: return jsonify({"ok": False})
        try:    p.status = "online" if get_client(p).ping() else "offline"
        except: p.status = "offline"
    ...
    def admin_panel_login_test(pid):
        """Test login to a panel and return detailed result."""
        p = db.session.get(Panel, pid)
        if not p: return jsonify({"ok": False, "error": "Not found"})
        evict_client(pid)  # force fresh client
        client = get_client(p)
        ok = client.login()
        return jsonify({"ok": ok, "logged_in": ok,
                        "message": "Login successful" if ok else "Login failed — check credentials"})
...
def _seed_admin():
    ...
    if not User.query.filter_by(email=email).first():
        ...
        db.session.add(u)
        db.session.commit()
        print(f"[SigmaFetcher] Admin created: {email}")
    else:
        print(f"[SigmaFetcher] Admin exists: {email}")
